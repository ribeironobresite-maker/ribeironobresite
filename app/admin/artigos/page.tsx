...
import { deleteExistingPost, saveArtigosCount } from "./actions";
...
export default async function PostsListPage({
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-dark" style={{ opacity: 0.7 }}>
            Posts publicados e rascunhos do seu blog.
          </p>
        </div>
      ...
      <form
        action={saveArtigosCount}
        className="mb-6 flex items-center gap-4 rounded-xl border bg-page px-5 py-4"
        style={{ borderColor: "var(--border-soft)" }}
      >
        <label className="text-sm text-dark font-medium shrink-0" htmlFor="artigosCount">
        ...
        <input
          ...
          min={1}
          max={20}
          defaultValue={settings.artigosCount}
          className="w-20 rounded-lg border px-3 py-1.5 text-sm bg-page text-dark focus:outline-none focus:ring-2 text-center"
          style={{ borderColor: "var(--border-soft)" }}
        />
        <button
          type="submit"
          className="btn-primary rounded-full px-5 py-1.5 text-sm font-semibold"
        >
          Salvar
        </button>
        <p className="text-xs text-dark" style={{ opacity: 0.55 }}>
          2 aparecem por vez — o visitante navega com as setas.
        </p>
      </form>
