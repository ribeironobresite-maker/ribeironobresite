import { getSiteSettings, buildOfficeFromSettings } from "@/lib/settings";
import WhatsAppCTA from "./WhatsAppCTA";

export default async function Contato() {
  const settings = await getSiteSettings();
  const office = buildOfficeFromSettings(settings);
  const email = settings.contactEmail;
  const mapsEnabled = settings.contactMapsEnabled && !!settings.contactMapsUrl;

  return (
    <section
      id="contato"
      className="py-16 md:py-24 bg-dark text-light relative overflow-hidden"
    >
      <div className="max-w-3xl mx-auto px-6 relative z-10">
        <div className="text-center mb-12">
          <div className="text-[11px] uppercase tracking-[0.3em] text-accent mb-4">
            Vamos conversar
          </div>
          <h2 className="font-serif text-3xl md:text-5xl mb-6 leading-tight">
            O primeiro passo
            <br />é uma <span className="italic text-accent">conversa.</span>
          </h2>
          <p className="text-light-soft max-w-xl mx-auto leading-relaxed">
            Conte sua situação no WhatsApp e receba orientação direta do
            advogado, sem compromisso.
          </p>
        </div>

        <div
          className="rounded-2xl p-7 md:p-8 border mb-8 bg-dark-2"
          style={{ borderColor: "var(--border-soft-dark)" }}
        >
          <div className="text-[10px] uppercase tracking-[0.3em] text-accent mb-3">
            {office.city} · {office.state}
          </div>
          <h3 className="font-serif text-2xl mb-4">Dr. Antonio C Cardozo</h3>
          <div className="space-y-2 text-sm text-light-soft mb-6">
            <div>{office.address}</div>
            <div>
              {office.neighborhood} · {office.city} · {office.state}
            </div>
          </div>
          <WhatsAppCTA
            officeData={office}
            className="inline-flex items-center gap-2 btn-primary px-6 py-3 rounded-full font-semibold text-sm w-full justify-center"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20.52 3.48A12 12 0 003.45 20.42L2 22l1.66-1.42a12 12 0 0016.86-17.1zM12 20a8 8 0 01-4.07-1.11l-.29-.17-3 .8.8-2.92-.18-.3A8 8 0 1112 20z" />
            </svg>
            WhatsApp · {office.whatsapp.display}
          </WhatsAppCTA>
        </div>

        {mapsEnabled && (
          <div className="mb-8 rounded-2xl overflow-hidden border" style={{ borderColor: "var(--border-soft-dark)" }}>
            <iframe
              src={settings.contactMapsUrl}
              width="100%"
              height="320"
              style={{ border: 0 }}
              allowFullScreen
              loading="lazy"
              referrerPolicy="no-referrer-when-downgrade"
              title={`Localização — ${office.city}`}
            />
          </div>
        )}

        <div className="text-center">
          <a
            href={`mailto:${email}`}
            className="inline-flex items-center gap-2 btn-outline-light px-7 py-4 rounded-full font-medium"
          >
            <svg
              className="w-5 h-5"
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              viewBox="0 0 24 24"
            >
              <path d="M3 8l9 6 9-6M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            {email}
          </a>
        </div>
      </div>
    </section>
  );
}
